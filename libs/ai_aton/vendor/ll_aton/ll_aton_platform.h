...
#define __LL_ATON_N6_DRIVERS_v0200                                                                                     \
  ((0x0 << 24U) | /* MAIN */                                                                                           \
   (0x2 << 16) |  /* SUB1 */                                                                                           \
   (0x0 << 8) |   /* SUB2 */                                                                                           \
   (0x0)          /* RC */                                                                                             \
  )
#define __LL_ATON_GET_VERSION_RC(x)          ((x)&0xFF)
#define __LL_ATON_GET_VERSION_FIRST_THREE(x) ((x) >> 8)
#define __LL_ATON_IS_AT_LEAST_VERSION(x, v)                                                                            \
  (((__LL_ATON_GET_VERSION_RC(x) == 0x0) &&                                                                            \
    (__LL_ATON_GET_VERSION_FIRST_THREE(x) >= __LL_ATON_GET_VERSION_FIRST_THREE(v))) ||                                 \
   (((__LL_ATON_GET_VERSION_RC(x) != 0x0) && (__LL_ATON_GET_VERSION_RC(v) == 0x0)) &&                                  \
    (__LL_ATON_GET_VERSION_FIRST_THREE(x) > __LL_ATON_GET_VERSION_FIRST_THREE(v))) ||                                  \
   (((__LL_ATON_GET_VERSION_RC(x) != 0x0) && (__LL_ATON_GET_VERSION_RC(v) != 0x0)) && ((x) >= (v))))

#if !__LL_ATON_IS_AT_LEAST_VERSION(__STM32N6xx_HAL_VERSION, __LL_ATON_N6_DRIVERS_v0200)
...
#else // __LL_ATON_IS_AT_LEAST_VERSION
...
#endif // __LL_ATON_IS_AT_LEAST_VERSION
...
/* See Excel table. No HAL definitions yet */
#define CDNN0_IRQn       102
#define CDNN1_IRQn       103
#define CDNN2_IRQn       104
#define CDNN3_IRQn       104
#define ATON_BASE        0x520E0000
...
#endif // !__LL_ATON_LIB_PHYSICAL_TO_VIRTUAL_ADDR

/* Default macro for virtual to physical address translation (direct mapping) */
#ifndef __LL_ATON_LIB_VIRTUAL_TO_PHYSICAL_ADDR
#define __LL_ATON_LIB_VIRTUAL_TO_PHYSICAL_ADDR(address) (address)
#endif // !__LL_ATON_LIB_VIRTUAL_TO_PHYSICAL_ADDR
...
#define LL_ATON_SHIFT(x) ((x) + 16)

// Generic boilerplate for enabling an ATON unit
#define ATON_ENABLE(unitname, id) LL_ATON_REG_WRITE_FIELD(unitname, id, CTRL, EN, 1)
...
#define ATON_DISABLE_CLR_CONFCLR(unitname, id)                                                                         \
  ...
  {                                                                                                                    \
    ...
    LL_ATON_REG_POLL(unitname, id, CTRL, CLR, 0);                                                                      \
                                                                                                                       \
    t = ATON_##unitname##_CTRL_DT;                                                                                     \
    t = ATON_##unitname##_CTRL_SET_CONFCLR(t, 1);                                                                      \
    ATON_##unitname##_CTRL_SET(id, t);                                                                                 \
    /* wait for unit to terminate clearing of configuration registers */                                               \
    LL_ATON_REG_POLL(unitname, id, CTRL, CONFCLR, 0);                                                                  \
  } while (0)

#if (LL_ATON_PLATFORM != LL_ATON_PLAT_EC_TRACE)

#define LL_ATON_REG_WRITE_RELOC(regaddr, base, offset)                                                                 \
  do                                                                                                                   \
  {                                                                                                                    \
    *(volatile uint32_t *)(uintptr_t)(regaddr) = (base) + (offset);                                                    \
  } while (0)
#define LL_ATON_REG_WRITE(regaddr, val)                                                                                \
  do                                                                                                                   \
  {                                                                                                                    \
    *(volatile uint32_t *)(uintptr_t)(regaddr) = (val);                                                                \
  } while (0)
#define LL_ATON_REG_WRITE_FIELD_RANGE(unitname, id, reg, start, fsize, val)                                            \
  do                                                                                                                   \
  {                                                                                                                    \
...
#define LL_ATON_REG_WRITE_FIELD(unitname, id, reg, field, val)                                                         \
  do                                                                                                                   \
  {                                                                                                                    \
    uint32_t t = ATON_##unitname##_##reg##_GET(id);                                                                    \
...
#define LL_ATON_REG_POLL(unitname, id, reg, field, val)                                                                \
  do                                                                                                                   \
  {                                                                                                                    \
...
#define LL_ATON_REG_WRITE(regaddr, val) ec_trace_write((uintptr_t)regaddr, (uint32_t)val)
#define LL_ATON_REG_WRITE_FIELD_RANGE(unitname, id, reg, start, fsize, val)                                            \
...
#define LL_ATON_REG_WRITE_FIELD(unitname, id, reg, field, val)                                                         \
  LL_ATON_REG_WRITE_FIELD_RANGE(unitname, id, reg, ATON_##unitname##_##reg##_##field##_LSB,                            \
