...
/* NuttX port (local deviation, see also libs/ai_aton/README-notes in the repo):
 *
 * Embedded builds compile the runtime with NDEBUG, which drops the assert()s -
 * essential here because the epoch error path runs in interrupt context, where
 * a failing assert() panics the target.  The error messages must not be lost
 * though: __LL_ATON_RT_IrqErr() prints the Epoch Controller state (EC_IRQ,
 * opcode counter, label) through LL_ATON_PRINTF(), and printf() cannot be used
 * from an ISR on NuttX (the console takes a mutex, which tripped
 * DEBUGASSERT(!up_interrupt_context()) in nxmutex_wait()).
 *
 * The messages are therefore routed into the OSAL's lock-free log ring and
 * dumped from task context afterwards (aton_model_dump_log()).
 */

extern void aton_osal_nuttx_log(const char *fmt, ...);
extern void aton_osal_nuttx_log_puts(const char *str);

#define LL_ATON_PRINTF(...)  aton_osal_nuttx_log(__VA_ARGS__)
#define LL_ATON_PUTS(str)    aton_osal_nuttx_log_puts(str)
#define LL_ATON_FFLUSH(...)  (0)
