...
#include "stm32n6xx.h"
/* NOTE: the original secure app includes "partition_stm32n6xx.h" for
 * TZ_SAU_Setup().  In the 38_SD_Card project all SAU_INIT_* macros are 0,
 * so TZ_SAU_Setup() is a no-op: we omit it here to match the bare-metal
 * startup environment exactly (SAU left at reset values). */
#include <math.h>
...
void SystemInit(void)
{
   ...
   * no-op in the bare-metal project (all SAU_INIT_* = 0), skipped. */
